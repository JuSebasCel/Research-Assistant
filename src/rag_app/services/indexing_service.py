...
class IndexingService:
    ...
    def __init__(
        ...
        Args:
            embedding_provider: Provider para generar embeddings
    ...
    def index_document(
        ...
        for i in range(0, len(chunks), self.batch_size):
            ...
            # Extraer contextualized_text de cada chunk
            # CRÍTICO: usar "passage:" prefix para indexar documentos
            texts = [chunk["contextualized_text"] for chunk in batch]
            
            # Generar embeddings con prefijo "passage:" (e5 requirement)
        ...
        logger.info(f"✓ Generados {len(all_embeddings)} embeddings")
    ...
    def search(
        ...
        # CRÍTICO: usar "query:" prefix para búsquedas
        query_embedding = self.embedding_provider.encode_single(
            query,
            mode="query",  # Esto añade "query: " automáticamente
        )
